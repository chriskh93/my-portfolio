# -*- coding: utf-8 -*-
"""
Created on Tue Sep 22 13:06:09 2020
...
# class cnn_class implements the condensed k-nearest neighbors classifier algorithm
...
class cnn_class:
    ...
    def tune(self):
        ...
        ypred=[]
        xinds=self.xinds
        z=pd.DataFrame(columns=df_tenp_0.columns)
        z1=pd.DataFrame()
        while(z.equals(z1)==False):
            z1=z.copy()
            for index,row in df_tenp_0.iterrows():
                if(len(z)==0):
                    z=z.append(row).reset_index(drop=True)
                else:
                    z['Distances']=0
                    endind=len(df_tenp_0.iloc[0,:])
                    for index1,row1 in z.iterrows():
                        x1=np.array(df_tenp_0.iloc[index,xinds])
                        x2=np.array(z.iloc[index1,xinds])
                        dist=self.eucd(x1,x2)
                        z.iloc[index1,endind]=dist
                    srted=z.sort_values('Distances')
                    if(srted.iloc[0,yind]!=df_tenp_0.iloc[index,yind]):
                        tba=df_tenp_0.iloc[index,:]
                        z=z.append(tba).reset_index(drop=True)
    ...
    # Method fit performs the condensed k-nearest neighbor classification on the dataset
    ...
    def fit(self):
        ...
        for ii in range(5):
            ...
            z=pd.DataFrame(columns=df_train.columns)
            z1=pd.DataFrame()
            while(z.equals(z1)==False):
                z1=z.copy()
                for index,row in df_train.iterrows():
                    ...
                    if(len(z)==0):
                        ...
                        for index1,row1 in z.iterrows():
                            ...
                            z.iloc[index1,endind]=dist
                        srted=z.sort_values('Distances')
                        if(srted.iloc[0,yinds]!=df_train.iloc[index,yinds]):
                            tba=df_train.iloc[index,:]
                            z=z.append(tba).reset_index(drop=True)

            df_train=z.reset_index(drop=True)
            ypred=[]
            ...
            start=end
            print("CNN Classifier Fold "+str(ii+1)+":")
            print("k="+str(k))
