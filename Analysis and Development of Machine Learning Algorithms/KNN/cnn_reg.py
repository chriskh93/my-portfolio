...
class cnn_reg:
    ...
    def tune(self):
        ...
        for err_th in np.arange(1,15,1):
            z=pd.DataFrame(columns=df_tenp.columns)
            z1=pd.DataFrame()
            while(z.equals(z1)==False):
                ...
                for index,row in df_tenp.iterrows():
                    if(len(z)==0):
                        ...
                        zd=z.copy()
                        zd=zd.reset_index(drop=True)
                        zd['Distances']=0
                        for index1,row1 in zd.iterrows():
                            zd.iloc[index1,-1]=self.eucd(zd.iloc[index1,self.xinds],df_tenp.iloc[index,self.xinds])
                        zd=zd.sort_values('Distances')
                        er=np.abs(zd.iloc[0,self.yinds]-df_tenp.iloc[index,self.yinds])/np.abs(df_tenp.iloc[index,self.yinds])
                        if(er*100>err_th):
                            ex=False
                            for index1,row1 in z.iterrows():
                                if(row1.equals(row)==True):
                                    ex=True
                                    break
                            if(ex==False):
                                z=z.append(row)
            ...
            wsss=[]
            bestkm=0
            unique_ys=z.iloc[:,yind].unique()
            bestcents=[]
            for km in np.arange(1,len(z),1):
                cents,df_2=self.kmeans(z,km)
                ...
                if(len(wsss)>1):
                    if(((np.abs(wsss[len(wsss)-1]-wsss[len(wsss)-2])/wsss[len(wsss)-2])<=0.15)):
                        ...
                        break
            cents,df_2=self.kmeans(z,bestkm)
        ...
        return bestkm,bsigma,best_err_th
    ...
    def fit(self):
        ...
        for ii in range(5):
            ...
            while(z.equals(z1)==False):
                ...
                for index,row in df_train.iterrows():
                    if(len(z)==0):
                        ...
                        zd['Distances']=0
                        for index1,row1 in zd.iterrows():
                            zd.iloc[index1,-1]=self.eucd(zd.iloc[index1,self.xinds],df_train.iloc[index,self.xinds])
                        zd=zd.sort_values('Distances')
                        er=np.abs(zd.iloc[0,self.yinds]-df_train.iloc[index,self.yinds])/np.abs(df_train.iloc[index,self.yinds])
                        if(er*100>err_th):
                            ex=False
                            for index1,row1 in z.iterrows():
                                if(row1.equals(row)==True):
            ...
            print("C-NN Regression Fold "+str(ii+1)+":")
            print("k="+str(k))
