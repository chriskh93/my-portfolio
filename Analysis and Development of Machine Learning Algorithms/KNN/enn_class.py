# -*- coding: utf-8 -*-
"""
Created on Tue Sep 22 12:08:37 2020
...
# class enn_class implements the edited k-nearest neighbors classifier algorithm
...
class enn_class:
        ...
        for k in np.arange(1,16,2):
            ...
            for index,row in df_tenp.iterrows():
                ...
                for index1,row1 in df_tenp_wd.iterrows():
                    x1=np.array(df_tenp.iloc[index,xinds])
            ...
            # Comparing the classes and removing the incorrect predictions:
            
            y=df_tenp.iloc[:,yind]
            for i in range(len(ypred)):
                if(ypred[i]!=y[i]):
                    df_tenp.drop(index=i,inplace=True)
            ypred=[]
            df_tenp_ed=df_tenp.reset_index(drop=True)
            df_tenp1=df_tenp_0.copy()
            
            # Re-testing the dataset with the edited training set:
            
            if(len(df_tenp_ed)>k):
                
                for index,row in df_tenp1.iterrows():
                    df_tenp_wd=df_tenp_ed.copy()
                    df_tenp_wd['Distances']=0
                    endind=-1
                    for index1,row1 in df_tenp_wd.iterrows():
                        x1=np.array(df_tenp1.iloc[index,xinds])
                        x2=np.array(df_tenp_wd.iloc[index1,xinds])
                        dist=self.eucd(x1,x2)
                        df_tenp_wd.iloc[index1,endind]=dist
                    srted=df_tenp_wd.sort_values('Distances').reset_index(drop=True)
                    
                    # Ensuring exemption of the variable itself from the nearest neighbors:
                    
                    strt_i=0
                    if(srted.iloc[0,-1]==0):
                        strt_i=1
                    srted=srted.iloc[strt_i:strt_i+k,:]
    ...
    # Method fit performs the edited k-nearest neighbor classification on the dataset
    ...
    def fit(self):
        ...
        for ii in range(5):
            ...
            # Performing k-nn classification on the training set with itself:
            ...
            for index,row in df_train.iterrows():
                ...
                for index1,row1 in df_train_wd.iterrows():
                    x1=np.array(df_train.iloc[index,xinds])
            ...
            y=df_train.iloc[:,yinds]
            for i in range(len(ypred)):
                if(ypred[i]!=y[i]):
                    df_train.drop(index=i,inplace=True)
            
            # Establishing the edited training set as the new training set to be
            #   used in predicting the values of the test set:
            ...
            start=end
            print("ENN Classifier Fold "+str(ii+1)+":")
            print("k="+str(k))
